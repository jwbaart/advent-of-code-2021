(() => {
    ...
    47 49 16 89  8
     2 95 48 38 85`;
    const givenTestDrawnBingoNumbers = `7,4,9,5,11,17,23,2,0,14,21,24,10,16,13,6,15,25,12,22,18,20,8,19,3,26,1`
    const givenTestBingoCards = `22 13 17 11  0
     8  2 23  4 24
    21  9 14 16  7
     6 10  3 18  5
     1 12 20 15 19
    ...
    const setFilterDrawnBingoNumber = (drawnBingoNumber) => (bingoCard: BingoCard) => bingoCard.map(bingoRow => bingoRow.filter(bingoNumber => bingoNumber !== drawnBingoNumber))
    const hasABingoCardEmptyRow = (bingoCards: BingoCard[]): boolean => bingoCards.map(bingoCard => bingoCard.filter(bingoRow => !!bingoRow.length)).some(bingoCard => bingoCard.length !== 5)
    const calculateBingoCardSum = (bingoCardSum: number, bingoRow: BingoRow) => bingoCardSum + bingoRow.reduce<number>((bingoRowSum, bingoNumber) => bingoRowSum + bingoNumber, 0)

    const drawnBingoNumbers = givenDrawnBingoNumbers.split(',').map(Number);
    let bingoCards = givenBingoCards
        .split(/\n/)
        .map(string => string.trim())
        .filter(String)
        .map(string => string.replaceAll('  ', ' '))
        .map(stringArray => stringArray.split(' ').map(Number))
        .reduce<BingoCard[]>((bingoCards, bingoCardRow, index) => {
            const bingoCardNumber = Math.floor(index / 5)
            if (bingoCards.length < bingoCardNumber + 1) {
                bingoCards.push([])
            }
            bingoCards[bingoCardNumber].push(bingoCardRow)
            return bingoCards
        }, [])
